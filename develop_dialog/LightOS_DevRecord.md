...
终于，断断续续一年，搓出了 LightOS，主要工作都完成于 2024年10月-12月 这三个月期间，这章总结写于 2024.12.22。

主要参考了踌躇月光的教程，这减少了我很多的调研和学习的时间，在此非常感谢大佬无私的分享。在具体实现过程中，我都是是随性而为，尤其是内存管理的部分，涉及到的页表配置以及高地址映射、地址布局等都是花了一番心思的。关于 MINIX V1 文件系统，也终于是吃透了。而关于硬件如 IDE 驱动、串口等的实现则是以学习为主。

虽然功能很基础，实现的也不是很精美，但是真的学到了好多东西，无论是内核还是用户程序加载执行的实现，都让我感受到了很多快乐和成就感，绝对是值得的。三个月的集中开发效率很高，截至目前，LightOS 共有 112 次 commit，github 上传情况如下图所示。

![image-20241222163718429](markdown_img\image-20241222163718429.png)

## 下阶段计划

后续如果有时间，可能针对目前欠缺的功能，完成下面几个大块功能。

* 支持 PF remap_pfn_range 实现懒加载。
* vmap 释放管理。
* 信号控制。
* 支持动态链接，并完善一个兼容 POSIX 的 libc，允许执行第三方编译的 elf。
* 网卡驱动。
* 学习 PCI 总线与设备管理。
* 学习并支持 UEFI 启动。
...
![ESC \[ 0;4;7m](../03%20字符输出/images/ansi_01.drawio.svg)
...
这块实在是懒得写了，也没什么难度，就是代码量大。我之前写的 console 还是挺精美的，但是不想改了，就直接抄 onix 了，这部分用的禁中断的方式，滚屏做的不够优雅，但是问题不大。 

# 定时器

有一些内核功能需要定时器支持，比如任务睡眠（当前实现是路径依赖的）、蜂鸣器超时、阻塞超时等。

本质是个 timer_t 数据结构，通过 node 组成链表，并且与 task 绑定，时间片中断时检查链表，超时则调用 handler。

sys_sleep 也解除路径依赖，改成基于 timer 的了。原理也很简单，但是阻塞超时懒得改了，先这样吧。
...
* https://zhuanlan.zhihu.com/p/563471531

总而言之，session 的概念与网络会话中的含义一致，都是一次与用户之间会话的概念。当前图形窗口中的终端窗口，都叫做**终端模拟器，或终端窗口**，而 linux 内核也提供几个默认的 tty 终端，可以使用 ctrl + alt +F1~7 来切换，而这个终端叫做**终端控制台**，与终端窗口的区别在于：

1. 终端窗口会创建虚拟 tty，与 session 绑定，session 退出时 tty 自然消亡。
2. 终端控制台在内核初始化时已经创建好 tty 了，session 退出 tty 仍然存在。

session 的意义是可以让多个 jobs 在一个终端中运行。在一个 session 中存在一个领头进程，且最多绑定一个 tty，也最多存在一个前台进程。tty 的标准输入输出都将与前台进程关联，而其他的 jobs 将以后台任务的形式运行。

当退出这个领头进程时，内核会检测 hangup 条件（就是挂断条件），从而将整个 session 下的所有任务一并退出。这就是 ssh 终端一旦退出则所有 jobs 都停止运行的原理。

所以可以使用`nohup`命令（no hangup），其不影响目标进程的 sid，但是会在挂断 session 后，将该进程托管给 systemd。
...
而信号，则是由 tty 生成的，传递给与当前 tty（终端）绑定的 session 的前台进程（进程组）。

当后台 job 中的进程试图从其控制终端读取数据时，通常会向进程组发送 SIGTTIN 信号。这通常会导致该组中的所有进程停止(变成 stopped 状态)。类似地，当后台 job 中的进程试图写入其控制终端时，默认行为是向进程组发送 SIGTTOU 信号，但是否允许写入的控制会更加的复杂。

PS：由于之前跳过了管道序列，所以进程组的概念也没有意义了。实现起来也很简单，在 lsh 中将管道序列中的不同进程打成同一组即可，即首个进程调用 setpgrp 设置为进程组首领，后续进程调用 setpgid 设置为相同进程组即可，也是 shell 的工作。我在内核实现了 setpgid，但是 shell 懒得改了。

## 问题

如果要杀死一个会话首领进程，该会话下所有进程都会死亡，但是如果有进程正在阻塞怎么办（如 I/O）？很多驱动的 waiter 的指针都是独立的，只清零 task_list 的指针是无效的，这些驱动最终将会唤醒这些已经死亡的进程，并尝试将数据读取到已经释放掉的用户缓冲区。难道需要将 read write 的系统调用增加判定，防止已经死亡的进程返回，或者读取到已经释放掉的用户空间，从而代理完成此次 I/O 防止出现 IDE 发生读取偏移 / 写入不完整的情况吗？

理论上这是信号等异常处理必须要面对的问题，即如何安全的停止 I/O。

目前的思路有两个：一个是程序死亡时驱动继续跑完内核态代码，但是不对用户态进行拷贝。第二个是将IO驱动完全分割，但这样涉及到多次拷贝，我觉得不合适。先暂时跳过，等看完信号机制之后可能就知道了。

# TTY

想要在已有的基础上增加一个 tty 层，其实本质上很简单。
