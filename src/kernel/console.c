...
#define MEM_BASE (0xB8000 + KERNEL_VADDR_OFFSET)  // 显卡内存起始位置
...
static _inline void insert_line(console_t* con) {
    ...
    erase_screen(con, (u16*)(con->screen + (con->y) * con->row_size),
                 con->width);
}
...
void console_init() {
    ...
    device_install(DEV_CHAR, DEV_CONSOLE, con, "console", 0, NULL, NULL,
                   console_write);
}