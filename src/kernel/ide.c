...
// static int32 ide_busy_wait(ide_ctrl_t* ctrl, u8 mask, int timeout_ms) {
...
int ide_pio_read(ide_disk_t* disk, void* buf, u8 count, idx_t lba) {
    ...
    mutex_lock(&ctrl->lock); // idle初始化也可以加锁，此时一定可以成功获取不会阻塞
...
//     outb(ctrl->bmbase + BM_COMMAND_REG,
//          inb(ctrl->bmbase + BM_COMMAND_REG) | BM_CR_START);
// }
...
// // 读取 atapi 数据包
// static int ide_atapi_packet_read_pio(ide_disk_t* disk,
//                                      u8* pkt,
//                                      int pktlen,
//                                      void* buf,
//                                      size_t bufsize) {
...
// static int ide_atapi_packet_read_dma(ide_disk_t* disk,
//                                      u8* pkt,
//                                      int pktlen,
//                                      void* buf,
//                                      size_t bufsize) {
...
static void ide_part_init(ide_disk_t* disk, u16* buf){
    ...
    for(size_t i = 0; i < IDE_PART_NR; ++i){
        ...
        if (entry->system == PART_FS_EXTENDED){
            ...
            eboot = (boot_sector_t*)(buf + SECTOR_SIZE); // 使用buf后半，防止覆盖boot
            ide_pio_read(disk, (void*)eboot, 1, entry->start);
...
//             if (disk->interface == IDE_INTERFACE_ATA) {
//                 dev_t dev =
//                     device_install(DEV_BLOCK, DEV_IDE_DISK, disk, disk->name,
//                     0,
...
//                     if (!part->count)
//                         continue;
//                     device_install(DEV_BLOCK, DEV_IDE_PART, part, part->name,
//                                    dev, ide_pio_part_ioctl,
//                                    ide_pio_part_read, ide_pio_part_write);
//                 }
//             } else if (disk->interface == IDE_INTERFACE_ATAPI) {
//                 device_install(DEV_BLOCK, DEV_IDE_CD, disk, disk->name, 0,
...
static void ide_ctrl_init() {
    ...
    for (size_t cidx = 0; cidx < IDE_CTRL_NR; cidx++) {
        ...
        ctrl->active = NULL;
        ctrl->waiter = NULL;
        ctrl->iobase = cidx?IDE_IOBASE_SECONDARY:IDE_IOBASE_PRIMARY; // 主从通道
        ctrl->control = inb(ctrl->iobase + IDE_CONTROL);
